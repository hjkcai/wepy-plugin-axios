...
import { buildURL, transformResponseHeaders } from './util'
...
export default function wepyAxiosAdapterFactory (axios) {
  ...
  // 否则就算用户有设置, 得到的 config 中也将不会有 upload 项
  axios.defaults['upload'] = undefined

  // 不使用默认的 transformRequest 和 transformResponse
    ...
    return enqueue((resolve, reject) => {
      ...
      request.success = response => {
        ...
        settle(resolve, reject, {
          ...
          headers: transformResponseHeaders(response.header),
          config,
          request
        })
      }
